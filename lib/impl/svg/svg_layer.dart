...
class SvgLayer extends SvgNode implements LayerImpl {
    ...
    SvgLayer(Layer shell, bool isReflection) : super(shell, isReflection) {
        shell.on('widthChanged', _onWidthChanged).on('heightChanged', _onHeightChanged).on('opacityChanged', _onOpacityChanged).on('stageSet', _onStageSet);
    }
    ...
    void _onWidthChanged(newValue) {
        ...
        (_element as SVG.SvgSvgElement).viewBox.baseVal..width = newValue / (getAttribute(SCALE_X, 1) * shell.stage.scaleX);
    }

    void _onHeightChanged(newValue) {
        _element.setAttribute(HEIGHT, '$newValue');
        (_element as SVG.SvgSvgElement).viewBox.baseVal..height = newValue / (getAttribute(SCALE_Y, 1) * shell.stage.scaleY);
    }
